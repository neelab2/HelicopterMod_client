class ActionHelicopterSwitchSeatsSIB: ActionBase
{
	void ActionHelicopterSwitchSeatsSIB()
	{
			...
			if ( Class.CastTo(trans, action_data.m_Target.GetObject()) )
			{
				...
				if ( seat >= 0 )
				{
					...
					if ( !GetGame().IsMultiplayer() || GetGame().IsClient() )
					{
						...
						action_data.m_Player.OnVehicleSeatDriverLeft();
					}
				}
			}
		}
	...
	override void End( ActionData action_data )
	{
		...
		{
		int nextSeat = trans.CrewPositionIndex( action_data.m_Target.GetComponentIndex() );
		vector matCrewTransformWS[4];
		trans.CrewTransform(nextSeat,matCrewTransformWS);
		action_data.m_Player.LinkToLocalSpaceOf( trans, matCrewTransformWS );
		}
	}
